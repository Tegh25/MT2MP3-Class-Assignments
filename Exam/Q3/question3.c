...
HashTable *init(int size) {
    ...
    new->size = size;
    new->arr = calloc(size, sizeof(SLL *));
...
void insert(HashTable *table, int key, int val) {
    ...
    node->next = table->arr[hash(key, table->size)];
    table->arr[hash(key, table->size)] = node;
}
...
int walkSLL(SLL *cur, int key) {
    ...
    while (search != NULL) {
        if (search == key) return search->val;
...
int lookup(HashTable *table, int key) {
    return walkSLL(table->arr[hash(key, table->size)], key);
}
